...
// 中文数字数组
static const char* cnNumbers[] = {"一","二","三","四","五","六","七","八","九","十"};

// 绘制控台模式界面 (400x300 分辨率)
// 布局：左上倒计时，右上图片 + 文字轮播，左下日历，右下大学目标
static void DrawDashboard(Adafruit_GFX* gfx, tm_t* tm, struct Lunar_Date* Lunar, gui_data_t* data) {
    // 屏幕分为四个区域
    const int16_t screen_w = data->width;   // 400
    const int16_t screen_h = data->height;  // 300
    const int16_t half_w = screen_w / 2;    // 200
    const int16_t half_h = screen_h / 2;    // 150
    
    // 1. 左上角：倒计时区域 (0,0 到 200,150)
    GFX_setFont(gfx, u8g2_font_wqy9_t_lunar);
    GFX_setTextColor(gfx, GFX_BLACK, GFX_WHITE);
    GFX_setCursor(gfx, 5, 12);
    GFX_printf(gfx, "倒计时");
    
    for (int i = 0; i < 2; i++) {
        if (data->dashboard.countdowns[i].event_name[0] != '\0') {
            uint32_t now_ts = data->timestamp;
            uint32_t target_ts = data->dashboard.countdowns[i].target_ts;
            int32_t days_left = (int32_t)(target_ts - now_ts) / 86400;
            if (days_left < 0) days_left = 0;
            
            GFX_setCursor(gfx, 5, 28 + i * 24);
            GFX_printf(gfx, "%s:%d 天", data->dashboard.countdowns[i].event_name, days_left);
        }
    }
    
    // 2. 右上角：图片 + 文字轮播区域 (200,0 到 400,150)
    // 默认显示小螃蟹图标位置 (中等大小约 48x48)
    int16_t icon_x = half_w + 20;  // 220
    int16_t icon_y = 15;
    int16_t icon_size = 48;
    
    // 绘制图标框 (实际项目中这里应该绘制存储的图标)
    GFX_drawRect(gfx, icon_x, icon_y, icon_size, icon_size, GFX_RED);
    GFX_fillRect(gfx, icon_x + 10, icon_y + 10, icon_size - 20, icon_size - 20, GFX_RED);
    
    // 显示当前轮播文字
    uint8_t msg_idx = data->dashboard.msg_index % 5;
    if (data->dashboard.messages[msg_idx].text[0] != '\0') {
        GFX_setFont(gfx, u8g2_font_wqy12_t_lunar);
        GFX_setTextColor(gfx, GFX_BLACK, GFX_WHITE);
        GFX_setCursor(gfx, half_w + 10, icon_y + icon_size + 20);
        GFX_printf(gfx, "%s", data->dashboard.messages[msg_idx].text);
    }
    
    // 3. 左下角：当月日历区域 (0,150 到 200,300)
    // 简化版日历：只显示数字和周几
    int16_t cal_x = 5;
    int16_t cal_y = 165;
    ...
    int16_t day_w = 26;  // 每列宽度
    for (int i = 0; i < 7; i++) {
        int16_t wx = cal_x + i * day_w;
        uint16_t color = (i == 0 || i == 6) ? GFX_RED : GFX_BLACK;
        GFX_setTextColor(gfx, color, GFX_WHITE);
        GFX_setCursor(gfx, wx, cal_y);
        GFX_printf(gfx, "%s", weekdays[i]);
    }
    ...
    // 绘制日期数字
    GFX_setFont(gfx, u8g2_font_helvB12_tn);
    for (uint8_t day = 1; day <= monthMaxDays && day <= 35; day++) {
        uint8_t row = (day + adjustedFirstDay - 1) / 7;
        uint8_t col = (day + adjustedFirstDay - 1) % 7;
        if (row >= 5) break;  // 最多 5 行
        
        int16_t dx = cal_x + col * day_w;
        int16_t dy = cal_y + 14 + row * 22;
        
        if (day > monthMaxDays) continue;
        
        uint16_t color = GFX_BLACK;
        // 周末红色
        int actualWeek = (firstDayWeek + day - 1) % 7;
        if (actualWeek == 0 || actualWeek == 6) color = GFX_RED;
        // 今天红色加粗
        if (day == tm->tm_mday) {
            GFX_setTextColor(gfx, GFX_WHITE, GFX_RED);
            GFX_fillRect(gfx, dx - 2, dy - 10, day_w - 2, 14, GFX_RED);
        } else {
            GFX_setTextColor(gfx, color, GFX_WHITE);
        }
        
        GFX_setCursor(gfx, dx, dy);
        GFX_printf(gfx, "%d", day);
    }
    
    // 4. 右下角：大学目标轮播区域 (200,150 到 400,300)
    GFX_setFont(gfx, u8g2_font_wqy9_t_lunar);
    GFX_setTextColor(gfx, GFX_BLACK, GFX_WHITE);
    GFX_setCursor(gfx, half_w + 5, 165);
    GFX_printf(gfx, "目标大学");
    
    uint8_t goal_idx = data->dashboard.goal_index % 5;
    // 显示当前目标（可以轮播显示多个）
    for (int i = 0; i < 3 && (goal_idx + i) < 5; i++) {
        uint8_t idx = (goal_idx + i) % 5;
        if (data->dashboard.goals[idx].univ_name[0] != '\0') {
            GFX_setCursor(gfx, half_w + 5, 182 + i * 28);
            GFX_setTextColor(gfx, GFX_BLACK, GFX_WHITE);
            GFX_printf(gfx, "%s", data->dashboard.goals[idx].univ_name);
            GFX_setTextColor(gfx, GFX_RED, GFX_WHITE);
            GFX_printf(gfx, " %d 分", data->dashboard.goals[idx].score);
            GFX_setTextColor(gfx, GFX_BLACK, GFX_WHITE);
            GFX_printf(gfx, " 第%s", cnNumbers[data->dashboard.goals[idx].rank]);
        }
    }
    
    // 绘制分隔线（可选，用户要求不要分割线，这里省略）
}
