...
void epd_config_init(epd_config_t* cfg) {
    ...
    // 初始化控台数据默认值
    epd_config_read(cfg);
    if (cfg->dashboard.countdowns[0].event_name[0] == 0xFF) {
        memset(&cfg->dashboard, 0, sizeof(dashboard_data_t));
        // 设置默认消息
        strcpy(cfg->dashboard.messages[0].text, "Hello");
        strcpy(cfg->dashboard.messages[1].text, "加油");
        strcpy(cfg->dashboard.messages[2].text, "早安");
        strcpy(cfg->dashboard.messages[3].text, "晚安");
        strcpy(cfg->dashboard.messages[4].text, "努力");
        epd_config_write(cfg);
    }
}
