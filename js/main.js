...
// ===== 控台模式(Dashboard)相关 =====
const DASH_COUNTDOWN_N = 2;
const DASH_MESSAGE_N = 5;
const DASH_GOAL_N = 5;
const DASH_TEXT_LEN = 20;
const cnNumbers = ['一','二','三','四','五','六','七','八','九','十'];

// 初始化控台配置输入框
function initDashboardInputs() {
  const cdDiv = document.getElementById('countdownInputs');
  const msgDiv = document.getElementById('messageInputs');
  const goalDiv = document.getElementById('goalInputs');
  if (!cdDiv) return; // 页面未加载完

  cdDiv.innerHTML = '';
  msgDiv.innerHTML = '';
  goalDiv.innerHTML = '';

  for (let i = 0; i < DASH_COUNTDOWN_N; i++) {
    cdDiv.innerHTML += `<div style="display:flex;gap:5px;align-items:center;">
      <span style="width:30px;">${i+1}.</span>
      <input type="text" id="cdEvent${i}" placeholder="事件名" style="flex:1;" maxlength="18">
      <input type="date" id="cdDate${i}" style="width:150px;">
    </div>`;
  }
  for (let i = 0; i < DASH_MESSAGE_N; i++) {
    msgDiv.innerHTML += `<div style="display:flex;gap:5px;align-items:center;">
      <span style="width:30px;">${i+1}.</span>
      <input type="text" id="msgText${i}" placeholder="文字" style="flex:1;" maxlength="18">
    </div>`;
  }
  for (let i = 0; i < DASH_GOAL_N; i++) {
    goalDiv.innerHTML += `<div style="display:flex;gap:5px;align-items:center;">
      <span style="width:30px;">${i+1}.</span>
      <input type="text" id="goalUniv${i}" placeholder="大学名" style="flex:1;" maxlength="18">
      <input type="number" id="goalScore${i}" placeholder="分数" style="width:80px;" min="0" max="999">
      <select id="goalRank${i}" style="width:60px;">
        ${cnNumbers.map((n,j) => `<option value="${j}">${n}</option>`).join('')}
      </select>
    </div>`;
  }
...
function loadDefaultDashboard() {
  ...
  const defaults = {
    countdowns: [
      {event: '新年', date: new Date(now.getFullYear()+1, 0, 1)},
      {event: '高考', date: new Date(now.getFullYear(), 5, 7)}
    ],
    messages: ['Hello', '加油', '早安', '晚安', '努力'],
    goals: [
      {univ: '清华大学', score: 680, rank: 0},
      {univ: '北京大学', score: 675, rank: 0},
      {univ: '复旦大学', score: 660, rank: 1},
      {univ: '浙江大学', score: 650, rank: 2},
      {univ: '上海交大', score: 655, rank: 1}
    ]
  };
  for (let i = 0; i < DASH_COUNTDOWN_N; i++) {
    document.getElementById('cdEvent'+i).value = defaults.countdowns[i].event;
    document.getElementById('cdDate'+i).value = defaults.countdowns[i].date.toISOString().split('T')[0];
  }
  for (let i = 0; i < DASH_MESSAGE_N; i++) {
    document.getElementById('msgText'+i).value = defaults.messages[i];
  }
  for (let i = 0; i < DASH_GOAL_N; i++) {
    document.getElementById('goalUniv'+i).value = defaults.goals[i].univ;
    document.getElementById('goalScore'+i).value = defaults.goals[i].score;
    document.getElementById('goalRank'+i).value = defaults.goals[i].rank;
  }
}

// 字符串转UTF-8字节数组(补齐到指定长度)
...
async function sendDashboardData() {
  ...
  for (let i = 0; i < DASH_COUNTDOWN_N; i++) {
    ...
    const eventBytes = strToBytes(event, DASH_TEXT_LEN);
    const payload = new Uint8Array(DASH_TEXT_LEN + 4);
    payload.set(eventBytes, 0);
    payload[DASH_TEXT_LEN] = (ts >> 24) & 0xFF;
    payload[DASH_TEXT_LEN+1] = (ts >> 16) & 0xFF;
    payload[DASH_TEXT_LEN+2] = (ts >> 8) & 0xFF;
    payload[DASH_TEXT_LEN+3] = ts & 0xFF;
    const cmd = new Uint8Array(3 + payload.length);
    cmd[0] = 0x40; cmd[1] = 0; cmd[2] = i;
    cmd.set(payload, 3);
    await write(cmd[0], cmd.slice(1));
    await new Promise(r => setTimeout(r, 100));
  }

  // 发送消息
  for (let i = 0; i < DASH_MESSAGE_N; i++) {
    const text = document.getElementById('msgText'+i).value || '';
    const textBytes = strToBytes(text, DASH_TEXT_LEN);
    const cmd = new Uint8Array(3 + DASH_TEXT_LEN);
    cmd[0] = 0x40; cmd[1] = 1; cmd[2] = i;
    cmd.set(textBytes, 3);
    await write(cmd[0], cmd.slice(1));
    await new Promise(r => setTimeout(r, 100));
  }
  ...
  // 发送大学目标
  for (let i = 0; i < DASH_GOAL_N; i++) {
    const univ = document.getElementById('goalUniv'+i).value || '';
    const score = parseInt(document.getElementById('goalScore'+i).value) || 0;
    const rank = parseInt(document.getElementById('goalRank'+i).value) || 0;
    const univBytes = strToBytes(univ, DASH_TEXT_LEN);
    const payload = new Uint8Array(DASH_TEXT_LEN + 3);
    payload.set(univBytes, 0);
    payload[DASH_TEXT_LEN] = score & 0xFF;
    payload[DASH_TEXT_LEN+1] = (score >> 8) & 0xFF;
    payload[DASH_TEXT_LEN+2] = rank;
    const cmd = new Uint8Array(3 + payload.length);
    cmd[0] = 0x40; cmd[1] = 2; cmd[2] = i;
    cmd.set(payload, 3);
    await write(cmd[0], cmd.slice(1));
    await new Promise(r => setTimeout(r, 100));
  }
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initDashboardInputs);
} else {
  initDashboardInputs();
}
